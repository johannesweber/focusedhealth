...
class FitBitPHP
{
    ...
    /**
     * Get user fat for specific date
     *
     * @throws FitBitException
     * @param  DateTime $date
     * @param  String $dateStr
     * @return mixed SimpleXMLElement or the value encoded in json as an object
     */
    public function getBodyFat($date, $dateStr = null)
    {
        $headers = $this->getHeaders();
        if (!isset($dateStr)) {
            $dateStr = $date;
        ...
        try {
            $this->oauth->fetch($this->baseApiUrl . "user/" . $this->userId . "/body/log/fat/date/" . $dateStr . "." . $this->responseFormat,
                null, OAUTH_HTTP_METHOD_GET, $headers);
        } catch (Exception $E) {
        }
